...
export function GalleryView() {
  ...
  const handleLightboxNav = useCallback((direction: 'prev' | 'next') => {
    ...
    if (direction === 'prev' && currentIndex > 0) {
      setLightbox(allItems[currentIndex - 1]);
    } else if (direction === 'next' && currentIndex < allItems.length - 1) {
      setLightbox(allItems[currentIndex + 1]);
    }
  }, [lightbox, images, markdownImages]);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[var(--bg-primary)]">
      ...
      {lightbox && (() => {
        ...
        const handlePrev = () => {
          if (hasPrev) setLightbox(allItems[currentIndex - 1]);
        };

        const handleNext = () => {
          if (hasNext) setLightbox(allItems[currentIndex + 1]);
        };
