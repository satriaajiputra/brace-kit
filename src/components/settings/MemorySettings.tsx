    ...
    <section className="flex flex-col gap-3 py-3 border-b border-border last:border-0">
      <div className="flex items-center justify-between px-0.5">
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-sm text-muted-foreground leading-none">Personalize assistant behavior</p>
        </div>
      ...
      <p className="text-sm text-muted-foreground leading-tight px-0.5 mb-1">
      ...
      <div className="sticky top-0 z-10 -mx-4 p-4 bg-background">
        {!showAddForm ? (
          <button
            className="w-full h-8 flex items-center justify-center gap-2 bg-secondary/60 hover:bg-secondary text-foreground text-sm font-medium rounded-md border border-border/50 transition-all shadow-sm group"
          ...
          <div className="flex flex-col gap-2 p-3 rounded-lg bg-secondary/30 border border-border/50 animate-in fade-in slide-in-from-top-2 duration-300">
            ...
            <textarea
              className="w-full min-h-[60px] p-2 text-sm bg-muted/40 border border-input rounded focus-visible:ring-1 focus-visible:ring-ring outline-none transition-all placeholder:text-muted-foreground/40 text-foreground resize-none"
              value={newMemoryContent}
              onChange={(e) => setNewMemoryContent(e.target.value)}
              placeholder="What should I remember?"
            />
            <div className="flex gap-2">
              <button className="flex-1 h-7 bg-primary text-primary-foreground text-sm font-medium rounded hover:bg-primary/90 transition-colors" onClick={handleAddMemory}>Save</button>
              <button className="flex-1 h-7 bg-muted text-muted-foreground text-sm font-medium rounded hover:bg-muted/80 transition-colors" onClick={() => { setShowAddForm(false); setNewMemoryContent(''); }}>Cancel</button>
            </div>
          </div>
        )}
        ...
        {store.memories.length > 0 && (
          <button
            className="mt-2 w-full h-8 flex items-center justify-center gap-2 bg-destructive/5 text-destructive hover:bg-destructive/10 text-xs font-bold uppercase tracking-wider rounded border border-destructive/20 transition-all"
      ...
      <div className="flex flex-col gap-4 mt-1">
        {store.memories.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-4 px-2 text-center rounded-lg border border-dashed border-border/50 bg-secondary/10">
            <p className="text-sm text-muted-foreground italic">No memories yet. Chat more to build personalization.</p>
          </div>
        ) : (
          MEMORY_CATEGORIES.map((cat) => {
            ...
            return (
              <div key={cat} className="flex flex-col gap-1.5">
                <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground/60 px-0.5">{MEMORY_CATEGORY_LABELS[cat as MemoryCategory]}</div>
                    ...
                    <div key={item.id} className="group relative flex items-start gap-2 p-2 rounded-md bg-secondary/30 hover:bg-secondary/40 border border-border/50 transition-all">
                      {editingMemory?.id === item.id ? (
                        <div className="flex-1 flex flex-col gap-2">
                          <textarea
                            className="w-full min-h-[50px] p-2 text-sm bg-muted/40 border border-input rounded focus-visible:ring-1 focus-visible:ring-ring outline-none transition-all text-foreground resize-none"
                            value={editContent}
                            onChange={(e) => setEditContent(e.target.value)}
                            autoFocus
                          />
                          <div className="flex gap-2 justify-end">
                            <button className="px-2 h-6 bg-primary text-primary-foreground text-xs font-bold uppercase rounded hover:bg-primary/90 transition-colors" onClick={handleSaveEdit}>Save</button>
                            <button className="px-2 h-6 bg-muted text-muted-foreground text-xs font-bold uppercase rounded hover:bg-muted/80 transition-colors" onClick={handleCancelEdit}>Cancel</button>
                          </div>
                        </div>
                      ) : (
                        <>
                          <span className="flex-1 text-sm text-foreground/90 leading-normal">{item.content}</span>
